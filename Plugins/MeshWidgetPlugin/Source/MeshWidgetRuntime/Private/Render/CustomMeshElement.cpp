...
FMatrix Compute2DScreenTransformMatrix(const FMatrix2x2& In2DTransformMatrix)
{
	FMatrix M = FMatrix::Identity;
	float A, B, C, D;
	In2DTransformMatrix.GetMatrix(A, B, C, D);
	M.M[0][0] = A;
	M.M[0][1] = -B;
	M.M[1][0] = -C;
	M.M[1][1] = D;
...
bool FCustomMeshSlateElement::BeginRenderingCanvas(const FIntRect& InCanvasRect, const FIntRect& InClippingRect, const FMatrix& MeshTransform, const FSlateRenderTransform& SlateTransform)
{
	...
	ENQUEUE_RENDER_COMMAND(SetupCustomMeshSlateRendering)([=](FRHICommandListImmediate& RHICmdList)
	{
...
{
	ENQUEUE_RENDER_COMMAND(UpdateCustomMeshSlateTimers)([=](FRHICommandListImmediate& RHICmdList)
	{
...
void FCustomMeshSlateElement::DrawRenderThread(FRHICommandListImmediate& RHICmdList, const void* InWindowBackBuffer)
{
	...
	FCanvasRenderContext RenderContext(GraphBuilder, RenderTarget.GetRenderTargetTexture(GraphBuilder), RenderTarget.GetViewRect(), RenderTarget.GetClippingRect());
	...
	DynamicPrimitiveUniformBuffer.Set(LocalToWS, LocalToWS, WSBounds, LocalBounds, LocalBounds, false, false, false, false, &CustomPrimitiveData);
